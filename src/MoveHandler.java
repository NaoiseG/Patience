public class MoveHandler {
    ...
    public MoveHandler(GameBoard gameBoard) {
        this.gameBoard = gameBoard;
    }

    public boolean checkLaneMove(int lane, Card card){
    ...
    public boolean checkSuitMove(int n, Card card){
    ...
    public void addToLane(int lane, int moves, Pile dealer){
        ...
        if(highest.isFaceDown()){ //Card isn't face up (Invalid move)
            gameBoard.printError("ERROR: NOT ALL REQUESTED CARDS ARE SHOWING"); //Not all showing
            return;
        }

        if (checkLaneMove(lane, highest)){
    ...
    public boolean addToSuitPile(int moves, int index, Pile dealer){
        if(moves > 1){
